...
# CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("DATABASE_URL".replace('postgres://', 'postgresql://'), "sqlite:///blog.db")
...
def load_user(user_id):
    return str(db.session.get(User, user_id))
...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("You've already registered with that email address.  Log in instead.")
